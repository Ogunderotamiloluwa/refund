...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btn-reset-submit').onclick = () => {
    ...
    if (pass.length < 7) { alert("Password must be at least 7 characters."); return; }
  ...
  ['btn-cancel-auth','btn-cancel-login','btn-forgot-cancel','btn-reset-cancel','btn-cancel-register'].forEach(id => {
  ...
  function advanceStep(step) {
    if(!loggedIn){ alert("You must login or register."); return; }
  ...
  ui.refundForm.onsubmit = async e => {
    ...
    data.calculatedRefund = 120; // Example
    ...
    msg.textContent = "Thank you! Your refund will be processed shortly.";
